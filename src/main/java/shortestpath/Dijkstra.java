    ...
    public Itinerary bestPath(Point2D.Double source, Point2D.Double destination, LocalTime startTime, NodeSize size, boolean walking) {
        ...
        while (!queue.isEmpty()) {
            ...
            try {
                ...
                List<Pair<Double, Station>> list = network.getClosestStations(currentNode.getCoordinates(), maxWalkingDistance);
                for (Pair<Double, Station> pair : list) {
                    Walk walk = new Walk(currentNode.getCoordinates(), pair.getValue().getCoordinates());
